...
#window size
window_size = 90
moving_avg_r = deque([0]*window_size, maxlen=window_size)
moving_avg_p = deque([0]*window_size, maxlen=window_size)
moving_avg_y = deque([0]*window_size, maxlen=window_size)
##############
# TASK 1 END #
##############
...
kalman_x.setSensorNoise(1, 0.1)  
kalman_y.setSensorNoise(1, 0.1)  
kalman_z.setSensorNoise(3, 0.3)  
#0.002, 0.01 gets rid of only the highest frequency noise but is quite fast
#0.1, 0.01 makes pretty quick response
#1, 0.1, very slow response
...
##kalman_x.setProcessNoise(0.01, 0.01)  
##kalman_y.setProcessNoise(0.01, 0.01)  
##kalman_z.setProcessNoise(0.01, 0.01)  

# kalman_x.setProcessNoise(0.000001, 0.000001)  
# kalman_y.setProcessNoise(0.000001, 0.000001)  
# kalman_z.setProcessNoise(0.000001, 0.000001)  

kalman_x.setProcessNoise(0.001, 0.001)  
kalman_y.setProcessNoise(0.001, 0.001)  
kalman_z.setProcessNoise(0.001, 0.001)  

#process_noise: lower>> less noise sensitivity means slower response
#sensor_noise: higher, less noise sensitivity means slower respons

#increasing these 100fold made it a lot more variant with noise
#decreasing 10fold made them quite a lot slower to respond, but less sensitive to noise
...
def moving_average(r, p, y):
    global moving_avg_r, moving_avg_p, moving_avg_y
    global rpy_moving_average

    # Add the new measurement
    # YOUR CODE HERE
    moving_avg_r.append(r)
    moving_avg_p.append(p)
    moving_avg_y.append(y)

    # Calculate average
    # YOUR CODE HERE
    rpy_moving_average = [sum(i)/window_size for i in [moving_avg_r, moving_avg_p, moving_avg_y]]
...
def complimentary_filter(r, p, y, gx, gy, gz, dt):
    ...
    # YOUR CODE HERE 
#gx - gyro roll rate in degrees per second
#gy - gyro pitch rate in degrees per second
#gz - gyro yaw rate in degrees per second
#dt - time difference between gyro measurements in seconds

    r1 = 0.05 * (rpy_complimentary[0] + gx * dt) + 0.95 * r
    p1 = 0.05 * (rpy_complimentary[1] + gy * dt) + 0.95 * p
    y1 = 0.05 * (rpy_complimentary[2] + gz * dt) + 0.95 * y

    # Filter output
    # Change the zeros to your output
    rpy_complimentary[0] = r1
    rpy_complimentary[1] = p1
    rpy_complimentary[2] = y1
